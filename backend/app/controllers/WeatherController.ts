...
import { SUPPORTED_CITIES } from '../entities/SupportedCities'
...
export class WeatherController {
  ...
  public async currentWeather(req: Request, res: Response, next: Next): Promise<void> {
    return this.fetchFromWeatherService(req, res, next, this.weatherService.fetchCurrentWeatherForCity, 'currentWeather')
  }

  @Get('/forecast/:cityId')
  public async forecast(req: Request, res: Response, next: Next): Promise<void> {
    return this.fetchFromWeatherService(req, res, next, this.weatherService.fetchForecastForCity, 'forecast')
  }
  ...
  private async fetchFromWeatherService(
    ...
    try {
      ...
      if (!valid) {
        res.json(400, { errors: `Please make sure that the cityId in path is a number and is one of ${permittedCities}` })
      } else {
        const response =
